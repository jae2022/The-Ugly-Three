...
const PRESETS = [
  ...
  { label: "다양한 방면을 바라보고싶어 🧐", values: { amo: 33, edi: 34, loco: 33 } },
];
  ...
  return (
    <section className="py-24 sm:py-32 px-5">
      <div className="max-w-5xl mx-auto">
        ...
        <motion.p {...fade(0.12)} className="mt-4 sm:mt-5 text-muted text-base sm:text-lg leading-relaxed max-w-2xl">
          기존에 당신이 AI에게 맞춰왔다면, 이제는 AI가 당신에게 맞춰요
          <br />
          Amo · Edi · Loco, 세 목소리의 비율을 정하면 셋이 함께 생각하고, 하나의 답으로 돌아올거예요
        </motion.p>

        {/* Demo Card */}
        <motion.div {...fade(0.18)} whileHover={{ boxShadow: "0 20px 48px rgba(0,0,0,0.10)", transition: { duration: 0.2 } }} className="mt-12 sm:mt-16 bg-white border border-black/[0.06] shadow-md rounded-3xl overflow-hidden">
          {/* Presets */}
          ...
          <div className="p-6 sm:p-7 border-b border-black/[0.05] flex flex-col gap-5">
            {PERSONAS.map(({ key, name, label, color }) => (
              <div key={key} className="flex items-center gap-3 sm:gap-4">
                ...
                />
                <span className="w-10 text-right text-sm font-mono font-medium text-foreground flex-shrink-0">{values[key]}%</span>
              </div>
            ))}
          </div>
